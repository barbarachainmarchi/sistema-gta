    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          {item.item_receita.length === 0 && <span className="text-[10px] text-orange-500/60">sem receita</span>}
        </div>
      </div>
...
export function CalculadoraClient({ userId, items, precos, lojas, lojaPrecos, favoritosIniciais }: Props) {
  ...
  // ref estável para toggleFavorito não reconstruir a cada render
  ...
  return (
    <div className="h-[calc(100vh-3rem)] flex overflow-hidden">
      ...
      {/* ── Coluna 2: Itens a produzir + Preços ── */}
          ...
          <div className="p-4 space-y-5">
            ...
            <div className="rounded-lg border border-border overflow-hidden">
              {batch.map(({ item_id, quantidade }) => {
                const item = itemMap[item_id]
                return (
                  <div key={item_id} className="flex items-center gap-2 px-3 py-2.5 border-b border-border/40 last:border-0">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate">{item?.nome ?? '—'}</div>
                      {item?.peso != null && (
                        <div className="text-[10px] text-muted-foreground">{fmtKg(item.peso * quantidade)} produzidos</div>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-border bg-white/[0.02] p-3">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Peso produzido</p>
                <p className="text-base font-bold mt-0.5">{totais.pesoProdutos > 0 ? fmtKg(totais.pesoProdutos) : '—'}</p>
              </div>
              <div className="rounded-lg border border-border bg-white/[0.02] p-3">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Peso ingredientes</p>
                <p className="text-base font-bold mt-0.5">{totais.pesoIngredientes > 0 ? fmtKg(totais.pesoIngredientes) : '—'}</p>
              </div>
            </div>
              ...
              return (
                <section className="rounded-lg border border-border bg-white/[0.02] p-4 space-y-3">
                    ...
                    <div className="space-y-1">
                      {linhas.map((l, i) => (
                        <div key={i} className="flex items-center justify-between text-xs">
                          <span className="text-muted-foreground truncate flex-1">{l.nome} ×{l.quantidade}</span>
                          <span className="tabular-nums ml-2">{l.totalLimpo != null ? fmt(l.totalLimpo) : '—'}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-3">
                    {totalSujo > 0 && (
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase">Sujo</p>
                        <p className="text-base font-semibold tabular-nums">{fmt(totalSujo)}</p>
                        {totais.custoTotal > 0 && (
                          <p className={cn('text-[10px] mt-0.5', (totalSujo - totais.custoTotal) >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                            margem {fmt(totalSujo - totais.custoTotal)}
                          </p>
                        )}
                      </div>
                    )}
                    {totalLimpo > 0 && (
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase">Limpo</p>
                        <p className="text-base font-semibold tabular-nums">{fmt(totalLimpo)}</p>
                      </div>
                    )}
                  </div>
                  <div className="pt-2 border-t border-border flex items-center gap-2">
                    ...
                    {fator != null && totalLimpo > 0 && (
                      <span className="text-sm font-semibold text-primary tabular-nums ml-auto">
                        {fmt(totalLimpo * fator)}
                      </span>
                    )}
                  </div>
                </section>
        ...
        {batch.length === 0 || ingredientesAgregados.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-xs text-muted-foreground text-center px-4">
              {batch.length === 0 ? 'Adicione itens para ver os ingredientes' : 'Nenhum item selecionado tem receita cadastrada'}
            </p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto">
                ...
                return (
                  <div key={ing.ingrediente_id} className="px-4 py-3 space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={cn('text-sm tabular-nums font-medium shrink-0', subtotal != null ? 'text-foreground' : 'text-muted-foreground')}>
                        {subtotal != null ? fmt(subtotal) : precoUnit != null ? fmt(precoUnit) : '—'}
                      </span>
                    </div>
            ...
            {totais.pesoIngredientes > 0 && (
              <div className="px-4 py-3 border-t border-border bg-white/[0.02]">
                ...
                {!totais.custoCompleto && totais.custoTotal > 0 && (
                  <p className="text-[10px] text-muted-foreground mt-1">* custo parcial (algumas lojas não selecionadas)</p>
                )}
              </div>
            )}
          </div>
