...
export function ConviteForm({ token, nomeSistema }: { token: string; nomeSistema: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!nomePersonagem.trim()) { toast.error('Nome no jogo é obrigatório'); return }
    setLoading(true)
    const res = await fetch('/api/convite/usar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token, apelido, senha, nomePersonagem: nomePersonagem.trim() }),
    })
        ...
        <div className="rounded-xl border border-border bg-card p-8 shadow-2xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="apelido" className="text-xs text-muted-foreground">Usuário</Label>
              <Input
                id="apelido"
                placeholder="seunome"
                value={apelido}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="nome" className="text-xs text-muted-foreground">Nome no jogo *</Label>
              <Input
                id="nome"
